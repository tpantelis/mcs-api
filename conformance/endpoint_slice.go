...
var _ = Describe("", Label(OptionalLabel, EndpointSliceLabel), func() {
	...
	Specify("Exporting a service should create an MCS EndpointSlice in the service's namespace in each cluster with the "+
		...
		for i, client := range clients {
			...
			Expect(eps).NotTo(BeNil(), reportNonConformant(fmt.Sprintf(
				"an MCS EndpointSlice was not found on cluster %d. A K8s EndpointSlice is distinguished from an MCS EndpointSlice "+
					"via the %q label where the former has the value set to %q. If the MCS implementation does not use EndpointSlices, "+
					"you can specify a Ginkgo label filter using the %q label where appropriate to skip this test.",
				i+1, discoveryv1.LabelManagedBy, K8sEndpointSliceManagedByName, EndpointSliceLabel)))
	...
	_ = wait.PollUntilContextTimeout(ctx, 100*time.Millisecond,
		20*time.Second, true, func(ctx context.Context) (bool, error) {
			...
			for i := range list.Items {
				...
				if eps.Labels[discoveryv1.LabelManagedBy] != K8sEndpointSliceManagedByName ||
					hasLabel(eps, v1alpha1.LabelServiceName) || hasLabel(eps, v1alpha1.LabelSourceCluster) {
					endpointSlice = eps
					return true, nil
				}
			}
