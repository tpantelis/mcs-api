...
var _ = Describe("", func() {
	...
	Context("Connectivity to an exported ClusterIP service", func() {
		Specify("should be accessible through DNS",
			Label(OptionalLabel, ConnectivityLabel, ClusterIPLabel), func() {
				...
				By("Issuing a request from all clusters", func() {
					...
					for _, client := range clients {
						...
						Expect(t.awaitCmdOutputContains(&client, command, "pod ip", -1)).To(Succeed(),
							reportNonConformant(""))
					}
				})
			})
	...
	Context("Connectivity to a ClusterIP service existing in two clusters but exported from one", func() {
		...
		It("should only access the exporting cluster",
			Label(OptionalLabel, ConnectivityLabel, ClusterIPLabel), func() {
				...
				for _, client := range clients {
					...
					for i := 1; i <= 20; i++ {
						Expect(t.awaitCmdOutputContains(&client, command, servicePodIP, i)).To(Succeed(), reportNonConformant(""))
					}
				}
			})
	})
