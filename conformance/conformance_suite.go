	...
	for _, client := range clients {
		serviceImport := t.awaitServiceImport(&client, t.helloService.Name, false,
			func(g Gomega, serviceImport *v1alpha1.ServiceImport) {
				g.Expect(len(serviceImport.Spec.IPs)+len(serviceImport.Spec.IPFamilies)).NotTo(BeZero(),
					"ServiceImport on cluster %q does not contain an IP or IP family", client.name)
			})

		ipFamilies := serviceImport.Spec.IPFamilies
		if len(ipFamilies) == 0 {
			ipFamilies = t.helloService.Spec.IPFamilies
		}
