...
func testClusterIPServiceImport() {
	...
	SpecifyWithSpecRef("An IP should be allocated for a ClusterSetIP ServiceImport for each IP family",
		...
		Label(RequiredLabel), func() {
			...
			// If IPFamilies is set on the ServiceImport, verify IPs are in the same order as IPFamilies
			if len(serviceImport.Spec.IPFamilies) > 0 {
